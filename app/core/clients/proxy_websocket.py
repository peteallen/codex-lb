...
from app.core.clients.account_http import get_account_websocket_proxy_uri
...
async def connect_responses_websocket(
    ...
    *,
    ...
    upstream_headers = _build_upstream_websocket_headers(headers, access_token, chatgpt_account_id or account_id)
    require_route_or_direct_egress_opt_in(
        route=route,
    ...
    account_proxy_url = await get_account_websocket_proxy_uri(account_id) if account_id else None
    ...
    async def _connect_websocket() -> ClientConnection:
    ...
    try:
        if account_proxy_url is not None and account_id:
            async with record_proxy_errors_for_account(account_id):
                response = await _connect_websocket()
        else:
            response = await _connect_websocket()
    ...
    return ArchivingResponsesWebSocket(
        ...
        headers=upstream_headers,
        account_id=account_id,
        direct_egress=allow_direct_egress and account_proxy_url is None,
    )
