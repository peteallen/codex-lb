...
@pytest.mark.asyncio
async def test_ensure_fresh_passes_account_id_to_refresh_jitter(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[dict[str, object]] = []

    def _fake_should_refresh(last_refresh: datetime, *, account_id: str | None = None) -> bool:
        calls.append({"last_refresh": last_refresh, "account_id": account_id})
        return False

    monkeypatch.setattr(auth_manager_module, "should_refresh", _fake_should_refresh)

    account = Account(
        id="acc_jitter",
        email="user@example.com",
        plan_type="pro",
        chatgpt_account_id="chatgpt_account_1",
    ...
    assert calls == [{"last_refresh": account.last_refresh, "account_id": account.id}]
...
@pytest.mark.asyncio
async def test_refresh_account_passes_account_id_for_direct_account_proxy(monkeypatch):
    ...
    async def direct_route(*_args: object, **_kwargs: object) -> None:
        return None
    ...
    async def _fake_refresh(_: str, **kwargs: object) -> TokenRefreshResult:
        ...
        return TokenRefreshResult(
            ...
            refresh_token="new-refresh",
            id_token="new-id",
            account_id="acc_direct_proxy",
            plan_type="pro",
            email="user@example.com",
        )

    monkeypatch.setattr(auth_manager_module, "get_background_session", fake_background_session)
    monkeypatch.setattr(auth_manager_module, "resolve_upstream_route", direct_route)
    monkeypatch.setattr(auth_manager_module, "refresh_access_token", _fake_refresh)
    ...
    assert captured_kwargs["account_id"] == "acc_direct_proxy"
    assert captured_kwargs["route"] is None
    assert captured_kwargs["allow_direct_egress"] is True
