...
@pytest.mark.asyncio
async def test_connect_responses_websocket_uses_account_proxy_before_env_proxy(monkeypatch):
    fake_connection = _FakeConnection()
    seen: dict[str, object] = {}
    ...
    async def fake_account_proxy_uri(account_id: str) -> str | None:
        assert account_id == "db-account-123"
        return "socks5://proxy-user:proxy-secret@proxy.example.test:1080"

    monkeypatch.setattr(proxy_websocket_module, "get_http_client", lambda: _UnexpectedHttpClient(), raising=False)
    monkeypatch.setattr(proxy_websocket_module, "websocket_connect", fake_websocket_connect, raising=False)
    monkeypatch.setattr(proxy_websocket_module, "get_account_websocket_proxy_uri", fake_account_proxy_uri)
    monkeypatch.setattr(
        proxy_websocket_module,
        "get_settings",
        lambda: SimpleNamespace(
    ...
    websocket = await connect_responses_websocket(
        ...
        "access-token",
        "db-account-123",
        chatgpt_account_id="chatgpt-account-456",
        allow_direct_egress=True,
    )

    kwargs = cast(dict[str, object], seen["kwargs"])
    assert kwargs["proxy"] == "socks5://proxy-user:proxy-secret@proxy.example.test:1080"
    headers = cast(dict[str, str], kwargs["additional_headers"])
    assert headers["chatgpt-account-id"] == "chatgpt-account-456"
    assert getattr(websocket, "upstream_proxy_route_mode") is None
...
@pytest.mark.asyncio
async def test_connect_responses_websocket_records_account_proxy_invalid_proxy(monkeypatch):
    recorded_accounts: list[str] = []

    async def fake_websocket_connect(url: str, **kwargs):
        del url, kwargs
        raise InvalidProxy("socks5://proxy.invalid", "unsupported proxy scheme")

    async def fake_account_proxy_uri(account_id: str) -> str | None:
        assert account_id == "account-123"
        return "socks5://proxy.invalid"

    @asynccontextmanager
    async def fake_record_proxy_errors(account_id: str) -> AsyncIterator[None]:
        try:
            yield
        except BaseException:
            recorded_accounts.append(account_id)
            raise

    monkeypatch.setattr(proxy_websocket_module, "get_http_client", lambda: _UnexpectedHttpClient(), raising=False)
    monkeypatch.setattr(proxy_websocket_module, "websocket_connect", fake_websocket_connect, raising=False)
    monkeypatch.setattr(proxy_websocket_module, "get_account_websocket_proxy_uri", fake_account_proxy_uri)
    monkeypatch.setattr(proxy_websocket_module, "record_proxy_errors_for_account", fake_record_proxy_errors)
    monkeypatch.setattr(
        proxy_websocket_module,
        "get_settings",
        lambda: SimpleNamespace(
    ...
    assert exc_info.value.status_code == 502
    assert _proxy_error_code(exc_info.value) == "upstream_unavailable"
    assert recorded_accounts == ["account-123"]
