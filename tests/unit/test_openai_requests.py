...
def test_compact_trims_oversized_input_by_estimated_tokens_with_head_tail_and_marker():
    ...
    # The marker must not promise that the most recent context survived: an
    # oversized optional terminal tool pair may be omitted atomically.
    assert "Required compact state anchors and retained input items remain in their original order" in marker_text
    assert "omitted items may include terminal context" in marker_text
...
def test_compact_trimming_rejects_latest_side_effect_tool_output_when_matching_call_cannot_fit():
    payload = {
        "model": "gpt-5.6-sol",
        "instructions": "",
        "input": [
