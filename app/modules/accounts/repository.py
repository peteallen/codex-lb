        ...
        *,
        ...
        if merge_by_chatgpt_identity and account.chatgpt_account_id:
            ...
            if canonical is not None:
                _apply_account_updates(canonical, account, include_proxy_fields=include_proxy_fields)
        ...
        *,
        ...
        if existing_by_id:
            if _same_unknown_workspace_identity(existing_by_id, account) and not preserve_unknown_workspace_duplicates:
                _apply_account_updates(existing_by_id, account, include_proxy_fields=include_proxy_fields)
                await self._session.commit()
            ...
            if existing_by_email:
                _apply_account_updates(existing_by_email, account, include_proxy_fields=include_proxy_fields)
                await self._session.commit()
                await self._session.refresh(existing_by_email)
                return existing_by_email
