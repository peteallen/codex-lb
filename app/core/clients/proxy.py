...
async def _stream_responses_with_session(
    ...
    if transport == "websocket":
        ...
        upstream_headers = _build_upstream_headers(headers, access_token, chatgpt_account_id or account_id)
        method = "POST"
...
async def thread_goal_request(
    ...
    *,
    ...
    upstream_headers = _build_upstream_headers(
        headers,
        access_token,
        chatgpt_account_id or account_id,
        accept="application/json",
    )
    ...
    try:
        ...
        if route is not None:
            ...
            if status_code >= 400:
                ...
                error_code, error_message = _error_details_from_envelope(error_payload)
                raise ProxyResponseError(status_code, error_payload, failure_phase="status")
            try:
